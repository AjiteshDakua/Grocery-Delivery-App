...
export default function DeliveryDashboard() {
  ...
  //   send location every 10s for deliveries
  useEffect(() => {
  ...
  const handleUpdateStatus = async (orderId: string, status: string) => {
    try {
      ...
      toast.success(`Delivery status updated to "${status}" successfully.`);
      fetchOrders();
    } catch (error: any) {
      toast.error(
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      toast.success("Delivery has been cancelled successfully.");
      setCancelModal(null);
      setCancelReason("");
      fetchOrders();
    } catch (error: any) {
      toast.error(
        error?.response?.data?.message ||
          "Failed to cancel the delivery. Please try again.",
      );
    } finally {
      setSubmitting(false);
    }
